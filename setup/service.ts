...
<dict>
    ...
    <string>${projectRoot}/logs/nanoharness.log</string>
    <key>StandardErrorPath</key>
    <string>${projectRoot}/logs/nanoharness.error.log</string>
</dict>
</plist>`;
  ...
  try {
    ...
    serviceLoaded = output.includes('com.nanoharness');
  } catch {
...
/**
 * Kill any orphaned nanoharness node processes left from previous runs or debugging.
...
function killOrphanedProcesses(projectRoot: string): void {
  try {
    ...
    logger.info('Stopped any orphaned nanoharness processes');
  } catch {
...
function setupSystemd(
  ...
  if (runningAsRoot) {
    unitPath = '/etc/systemd/system/nanoharness.service';
    ...
    unitPath = path.join(unitDir, 'nanoharness.service');
    systemctlPrefix = 'systemctl --user';
  }

  const unit = `[Unit]
Description=NanoHarness Personal Agent Harness
...
StandardOutput=append:${projectRoot}/logs/nanoharness.log
StandardError=append:${projectRoot}/logs/nanoharness.error.log
...
WantedBy=${runningAsRoot ? 'multi-user.target' : 'default.target'}`;
  ...
  // Kill orphaned nanoharness processes to avoid channel connection conflicts
  ...
  try {
    execSync(`${systemctlPrefix} enable nanoharness`, { stdio: 'ignore' });
  } catch (err) {
    logger.error({ err }, 'systemctl enable failed');
  }

  try {
    execSync(`${systemctlPrefix} start nanoharness`, { stdio: 'ignore' });
  } catch (err) {
  ...
  let serviceLoaded = false;
  try {
    execSync(`${systemctlPrefix} is-active nanoharness`, { stdio: 'ignore' });
    serviceLoaded = true;
  } catch {
    // Not active
  }
...
function setupNohupFallback(
  ...
  const wrapperPath = path.join(projectRoot, 'start-nanoharness.sh');
  const pidFile = path.join(projectRoot, 'nanoharness.pid');

  const lines = [
    '#!/bin/bash',
    '# start-nanoharness.sh — Start NanoHarness without systemd',
    ...
    '    echo "Stopping existing NanoHarness (PID $OLD_PID)..."',
    '    kill "$OLD_PID" 2>/dev/null || true',
    '    sleep 2',
    '  fi',
    'fi',
    '',
    'echo "Starting NanoHarness..."',
    `nohup ${JSON.stringify(nodePath)} ${JSON.stringify(projectRoot + '/dist/index.js')} \\`,
    `  >> ${JSON.stringify(projectRoot + '/logs/nanoharness.log')} \\`,
    `  2>> ${JSON.stringify(projectRoot + '/logs/nanoharness.error.log')} &`,
    '',
    `echo $! > ${JSON.stringify(pidFile)}`,
    'echo "NanoHarness started (PID $!)"',
    `echo "Logs: tail -f ${projectRoot}/logs/nanoharness.log"`,
  ];
